 ...
 */
public class GraphTemporalMotifMiner {
    
    private final Graph graph;
    private final long delta; // Finestra temporale massima (in secondi)
    
    public GraphTemporalMotifMiner(Graph graph, long delta) {
        this.graph = graph;
        this.delta = delta;
    }
    ...
    private void findMergedGiveAndTakePatterns(
        ...
        while (mergedIt.hasNext()) {
            ...
            // 2) FINE PER ARCO NON INCREMENTALE
            if(  (expectingOutgoing && !edge.isOutgoingFrom(center))  || 
                 (!expectingOutgoing && !edge.isIncomingTo(center)) ) {
    ...
    private void findMergedReceiveAndForwardPatterns(
        ...
        while (mergedIt.hasNext()) {
            ...
            // 2) FINE PER ARCO NON INCREMENTALE
            if(  (expectingIncoming && !edge.isIncomingTo(center))  || 
                 (!expectingIncoming && !edge.isOutgoingFrom(center)) ) {
