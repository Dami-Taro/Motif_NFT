...
public class TimeFormatter {
    ...
    public static String secondsToTruncatedString(long delta) {
        // Definizione delle costanti per ciascuna unità di tempo
        long secondsInYear = 365 * 24 * 60 * 60; // Anni (approssimazione di 365 giorni)
        long secondsInMonth = 30 * 24 * 60 * 60; // Mesi (approssimazione di 30 giorni)
        long secondsInWeek = 7 * 24 * 60 * 60; // Settimane (7 giorni)
        ...
        if (years > 0){     result.append(years).append("y"); if(months < 1) return result.toString(); } //  if(months < 1) return result.toString(); 
        if (months > 0) {   result.append(months).append("m"); if(weeks < 1) return result.toString(); } // if(weeks < 1) return result.toString(); }
        if (weeks > 0) {    result.append(weeks).append("w"); if(days < 1) return result.toString(); } //   if(days < 1) return result.toString(); }
        if (days > 0) {     result.append(days).append("d"); if(hours < 1) return result.toString(); } //    if(hours < 1) return result.toString(); }
        if (hours > 0) {    result.append(hours).append("h"); if(minutes < 1) return result.toString(); } //   if(minutes < 1) return result.toString(); }
        if (minutes > 0) {  result.append(minutes).append("min"); if(seconds < 1) return result.toString(); } // if(seconds < 1) return result.toString(); }
